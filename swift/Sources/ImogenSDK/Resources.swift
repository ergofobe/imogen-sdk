...
/// knows nothing at all reads the code out of it and calls ``claim(_:)``. Between them the
/// device learns where the server is and gets an authorization code for it, in one
/// gesture.
///
/// What ``claim(_:)`` returns is an ordinary OAuth code bound to a PKCE challenge the
/// device generated, so a photographed QR code is not on its own enough to reach a
/// library. ``OAuthClient/pair(pairingCode:clientName:redirectURI:deviceName:scopes:)``
...
public struct Pairing: Sendable {
    ...
    public func create() async throws -> PairingTicket {
        try await http.request("POST", "/api/v1/pairing")
    }

    /// Whether a device has taken the ticket yet, and what it called itself.
    public func status(_ ticketId: String) async throws -> PairingStatus {
        try await http.request("GET", "/api/v1/pairing/\(ticketId)")
    }

    /// Spends a ticket. Called by the device, not by the browser that made it.
    public func claim(_ request: PairingClaimRequest) async throws -> PairingClaim {
        try await http.request(
